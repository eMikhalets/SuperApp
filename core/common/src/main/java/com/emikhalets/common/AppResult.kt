...
class AppResult<out T> constructor(val value: Any?) {
    ...
    companion object {
        ...
        fun <T> success(
            data: T?,
            code: Int = AppConstant.DEFAULT_SUCCESS_CODE,
        ): AppResult<T> = AppResult(createSuccess(code, data))

        fun <T> failure(
            message: String?,
            code: Int = AppConstant.DEFAULT_FAILURE_CODE,
        ): AppResult<T> = AppResult(createFailure(code, message ?: ""))
    }
